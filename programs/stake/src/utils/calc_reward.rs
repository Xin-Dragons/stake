...
use crate::{StakeError, STAKING_ENDS};
...
pub fn calc_reward(
    ...
    let cutoff_time = i64::min(current_time, staking_ends_at.unwrap_or(STAKING_ENDS));
    ...
    for ix in index..=total_changes {
        let big_num = if ix == total_changes {
            cutoff_time
        } else {
            reward_change_time[ix + 1]
        };
        let sml_num = if ix == index {
            staked_at
        } else {
            reward_change_time[ix]
        };

        let rewardable_time = big_num
            .checked_sub(sml_num)
        ...
        let reward = rewardable_time
            .checked_mul(reward_emission[ix])
